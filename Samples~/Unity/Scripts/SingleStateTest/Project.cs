        ...
        public override IEnumerable<ILab<Model>> CreateLabs(Model model)
        {
            ...
            switch (GetState(model))
            {
                ...
                case KickboardState.Mounted:
                    labs.Add(new("Mount_Licensed")
                    {
                        Actor = m => Assert.Throws<InvalidOperationException>(() => m.Kickboard.Mount(new Rider(true))),
                        ToUncontinuable = true
                    });
                    if (model.Kickboard.Rider != model.TargetedRider) labs.Add(new()
                    {
                        ID = "Mount_Targeted",
                        Actor = m => Assert.Throws<InvalidOperationException>(() => m.Kickboard.Mount(m.TargetedRider)),
                        ToUncontinuable = true
                    });
                    labs.Add(new("Mount_Same")
                    {
                        Actor = m => Assert.DoesNotThrow(() => m.Kickboard.Mount(m.rider)),
                        Asserter = Check
                    });
                    labs.Add(new("Mount_NotLicensed")
                    {
                ...
                case KickboardState.Disposed:
                    labs.Add(new("Mount_Licensed")
                    {
                        Actor = m => Assert.Throws<ObjectDisposedException>(() => m.Kickboard.Mount(new Rider(true))),
                        Asserter = Check
                    });
                    labs.Add(new("Mount_Targeted")
                    {
                        Actor = m => Assert.Throws<ObjectDisposedException>(() => m.Kickboard.Mount(m.TargetedRider)),
                        Asserter = Check
                    });
                    labs.Add(new("Mount_NotLicensed")
                    {
                        Actor = m => Assert.Throws<ObjectDisposedException>(() => m.Kickboard.Mount(new Rider(false))),
                        Asserter = Check
                    });
                    labs.Add(new("Mount_Null")
                    {
                        Actor = m => Assert.DoesNotThrow(() => m.Kickboard.Mount(null)),
                        Asserter = Check
                    });
                    break;
            }
            ...
            switch (GetState(model))
            {
                case KickboardState.Idle:
                    labs.Add(new("Ride")
                    {
                        Actor = m => Assert.Throws<InvalidOperationException>(() => m.Kickboard.Ride()),
                        ToUncontinuable = true
                    });
                    break;
                ...
                case KickboardState.Disposed:
                    labs.Add(new("Ride")
                    {
                        Actor = model => Assert.Throws<ObjectDisposedException>(() => model.Kickboard.Ride()),
                        Asserter = Check
                    });
                    break;
            }
            ...
            switch (GetState(model))
            {
                ...
                case KickboardState.Disposed:
                    labs.Add(new("Dismount")
                    {
                        Actor = model => Assert.DoesNotThrow(() => model.Kickboard.Dismount()),
                        Asserter = Check
                    });
                    break;
            }
