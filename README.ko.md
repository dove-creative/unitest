...
일반 유닛 테스트가 개별 테스트 케이스를 직접 나열하는 데 집중한다면, UniTest는 현재 `Model`에서 가능한 `Lab`을 생성하고 실행 이력에 따라 후속 상태를 확장한다. 또한 에이전트를 위한 테스트 작성과 CLI 실행 자동화 워크플로도 함께 제공한다.
...
에이전트가 UniTest 기반 테스트를 작성하거나 도메인별 Unity 밖 POCO 테스트 실행 경로를 정리할 때는 `Documentation~/Workflow.ko`의 워크플로 문서를 따른다.
